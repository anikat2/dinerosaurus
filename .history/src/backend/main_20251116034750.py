...
async def del_amt(amount):
    total_portfolio_value -= float(amount)
    db.reference('/balance').set({'amount': total_portfolio_value})
    return {"status": "success", "new_balance": round(total_portfolio_value, 2)}