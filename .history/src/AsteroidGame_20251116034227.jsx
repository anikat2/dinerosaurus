...
export default function AsteroidGame({
  ...
  // Debug: Check what balance contains
  useEffect(() => {
  ...
  const moveAsteroid = () => {
    ...
    setPosition(prev => {
      ...
      if (isCollision) setGameOver(true);
  ...
  return (
    <div className="asteroid-layout">
      ...
      <div style={{ marginTop: "10px" }}>
        Balance: ${typeof balance === 'number' ? balance.toFixed(2) : (balance?.total_account_value?.toFixed(2) || '0.00')}
      </div>
    </div>
  );
}